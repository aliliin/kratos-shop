...
type OrderPay struct {
	...
	TradeNo   string    `gorm:"type:varchar(100) comment '交易号、流水号'"`        // 交易号就是支付宝的订单号 查账
	PayType   string    `gorm:"type:int comment '1alipay(支付宝)， 2wechat(微信)'"`
	PayStatus string    `gorm:"type:type:tinyint(1) unsigned not null;default:0;comment '1PAYING(待支付), 2TRADE_SUCCESS(成功)， 3TRADE_CLOSED(超时关闭), 4WAIT_BUYER_PAY(交易创建), 5TRADE_FINISHED(交易结束)'"`
